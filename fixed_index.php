...
<body>
    ...
    <script>
        // Fixed version with correct login field
        ...
        // Fixed login with correct field name
        async function login(username, password) {
            try {
                const res = await fetch('/muzic2/src/api/login.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ login: username, password: password }), // Use 'login' field
                    credentials: 'include'
                });
                const data = await res.json();
                if (res.ok) {
                    ...
                    alert('Ошибка входа: ' + (data.message || data.error));
                }
            } catch (e) {
                alert('Ошибка сети');
            }
