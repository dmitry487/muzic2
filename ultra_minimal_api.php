<?php
// Ultra minimal API - no checks, no validation, just data
session_start();
...
switch ($action) {
    case 'home':
        ...
        $albums = $db->query("SELECT DISTINCT album as title, artist, cover FROM tracks WHERE album IS NOT NULL LIMIT 5")->fetchAll();
        $artists = $db->query("SELECT DISTINCT artist as name FROM tracks WHERE artist IS NOT NULL LIMIT 5")->fetchAll();
