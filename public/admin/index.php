...
if (isset($_GET['api'])) {
  ...
  // Simple server-side logging to diagnose admin API issues
  function __admin_log($m){
    try{ $f=__DIR__.'/admin_api.log'; $p='['.date('Y-m-d H:i:s').'] '.($_SERVER['REQUEST_URI']??'')." "; if(is_array($m)||is_object($m)){$m=json_encode($m,JSON_UNESCAPED_UNICODE);} @file_put_contents($f,$p.$m."\n",FILE_APPEND);}catch(Throwable $e){}
  }
  ...
  if ($method !== 'GET') {
    ...
    __admin_log(['entity'=>$entity,'method'=>$method,'raw'=>$raw]);
  }
  try {
    if ($entity === 'artists') {
      if ($method === 'GET') {
    ...
    if ($entity === 'tracks') {
      // Ensure optional video_url column exists
      ...
      if ($method === 'GET') {
        ...
        if ($albumExact !== '') {
          $st=$db->prepare('SELECT * FROM tracks WHERE TRIM(LOWER(album))=TRIM(LOWER(?)) ORDER BY id ASC LIMIT 1000');
      ...
      if ($action === 'create') {
        ...
        $cover=trim((string)($body['cover']??'')); $type=in_array(($body['album_type']??'album'),['album','ep','single'])?$body['album_type']:'album'; $dur=intval($body['duration']??0); $video_url=trim((string)($body['video_url']??''));
        ...
        $st->execute([$title,$artist,$album,$type,$dur,$file,$cover,$video_url]); echo json_encode(['success'=>true,'id'=>$db->lastInsertId()]); exit;
      }
      if ($action === 'update') {
        $id=intval($body['id']??0); if ($id<=0) throw new Exception('Неверный ID');
        $fields=['title','artist','album','album_type','duration','file_path','cover','video_url']; $set=[]; $params=[':id'=>$id];
    ...
    __admin_log(['error'=>$e->getMessage(),'trace'=>$e->getTraceAsString()]);
...
function normVideo(v){ v=(v||'').trim(); if(!v) return ''; if(/^https?:/i.test(v) || v.startsWith('/')) return v; const i=v.indexOf('tracks/'); return '/muzic2/public/src/api/video.php?f='+encodeURIComponent(i!==-1?v.slice(i):v.replace(/^\/+/,'')); }
function openVideoPreview(raw){ const url=normVideo(raw); let overlay=document.getElementById('video-preview'); if(!overlay){ overlay=document.createElement('div'); overlay.id='video-preview'; overlay.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.6);display:flex;align-items:center;justify-content:center;z-index:100000'; const box=document.createElement('div'); box.style.cssText='background:#0f0f0f;border:1px solid #2a2a2a;border-radius:12px;max-width:800px;width:92%;padding:8px;position:relative'; const close=document.createElement('button'); close.textContent='×'; close.title='Закрыть'; close.style.cssText='position:absolute;top:6px;right:6px;width:28px;height:28px;border:none;border-radius:50%;background:#2a2a2a;color:#b3b3b3;cursor:pointer'; close.onclick=()=>{ try{v.pause();}catch(_){} document.body.removeChild(overlay); }; const v=document.createElement('video'); v.id='video-preview-player'; v.style.cssText='width:100%;height:auto;max-height:70vh;background:#000'; v.controls=true; box.appendChild(close); box.appendChild(v); overlay.appendChild(box); document.body.appendChild(overlay); } const vp=document.getElementById('video-preview-player'); if(vp){ try{ while(vp.firstChild) vp.removeChild(vp.firstChild);}catch(_){} const s=document.createElement('source'); s.src=encodeURI(url); s.type= (url.toLowerCase().endsWith('.webm')?'video/webm':'video/mp4'); vp.appendChild(s); vp.load(); setTimeout(()=>{ try{vp.play().catch(()=>{});}catch(_){} },0); } }
...
async function delTrack(t){ if(!confirm(`Удалить трек "${t.title}"?`)) return; const r=await fetch(`${api}&entity=tracks`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete',id:t.id})}); const j=await r.json(); if(j.success){ ok('Трек удалён'); load(); } else { err(j.message||'Ошибка') } }
 add.onclick=()=>{ if(tab==='artists'){ editArtist({artist:'',artist_cover:'',bio:''}); document.getElementById('f-name').value=''; saveBtn.onclick = async ()=>{ try{ mok('Сохранение...'); let cover=document.getElementById('f-cover').value.trim(); const file=document.getElementById('f-file').files[0]; if(file){ cover=await upload(file); document.getElementById('f-cover').value=cover; } const payload={action:'create', name:document.getElementById('f-name').value.trim(), cover, bio:document.getElementById('f-bio').value.trim()}; const r=await fetch(`${api}&entity=artists`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); const j=await r.json(); if(!j.success) throw new Error(j.message||'Ошибка'); closeModal(); load(); }catch(e){ merr(e.message) } } }
 ...
 else if(tab==='tracks'){ editTrack({id:0,title:'',artist:'',album:'',album_type:'album',file_path:'',cover:'',duration:0}); const originalHandler = saveBtn.onclick; saveBtn.onclick = async ()=>{ try{ // override to create when id is 0
      mok('Сохранение...');
      ...
      const payload={action:'create', title:document.getElementById('f-title').value.trim(), artist:document.getElementById('f-artist').value.trim(), album:document.getElementById('f-album').value.trim(), album_type:document.getElementById('f-type').value, file_path:document.getElementById('f-file').value.trim(), cover, duration:parseInt(document.getElementById('f-dur').value||0,10)};
