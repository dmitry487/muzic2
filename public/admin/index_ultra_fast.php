  ...
  try {
    if ($entity === 'artists') {
      if ($method === 'GET') {
        ...
        if ($q !== '') {
          $st = $db->prepare("SELECT a.name AS artist, a.cover AS artist_cover, a.bio AS bio, COUNT(t.id) AS tracks, MIN(t.cover) AS track_cover FROM artists a LEFT JOIN tracks t ON TRIM(LOWER(a.name))=TRIM(LOWER(t.artist)) WHERE a.name LIKE ? GROUP BY a.name, a.cover, a.bio ORDER BY a.name ASC LIMIT 50");
          $st->execute(['%'.$q.'%']);
          $rows = $st->fetchAll();
        } else {
          $rows = $db->query("SELECT a.name AS artist, a.cover AS artist_cover, a.bio AS bio, COUNT(t.id) AS tracks, MIN(t.cover) AS track_cover FROM artists a LEFT JOIN tracks t ON TRIM(LOWER(a.name))=TRIM(LOWER(t.artist)) GROUP BY a.name, a.cover, a.bio ORDER BY a.name ASC LIMIT 25")->fetchAll();
        }
    ...
    if ($entity === 'albums') {
      if ($method === 'GET') {
        $q = trim((string)($_GET['q'] ?? ''));
        if ($q!=='') {
          $st = $db->prepare('SELECT album, MIN(artist) AS artist, MIN(album_type) AS album_type, MIN(cover) AS cover, COUNT(*) AS track_count FROM tracks WHERE album LIKE ? GROUP BY album ORDER BY album ASC LIMIT 50');
          $st->execute(['%'.$q.'%']);
          $rows = $st->fetchAll();
        } else {
          $rows = $db->query('SELECT album, MIN(artist) AS artist, MIN(album_type) AS album_type, MIN(cover) AS cover, COUNT(*) AS track_count FROM tracks GROUP BY album ORDER BY album ASC LIMIT 25')->fetchAll();
        }
        echo json_encode(['success'=>true,'data'=>$rows], JSON_UNESCAPED_UNICODE); 
        exit;
      }
    ...
    if ($entity === 'tracks') {
      ...
      if ($method === 'GET') {
        ...
        if ($albumExact !== '') {
          $st=$db->prepare('SELECT * FROM tracks WHERE TRIM(LOWER(album))=TRIM(LOWER(?)) ORDER BY id ASC LIMIT 100');
          $st->execute([$albumExact]);
          $rows = $st->fetchAll();
        } elseif ($q!=='') {
          $st=$db->prepare('SELECT * FROM tracks WHERE title LIKE ? OR artist LIKE ? OR album LIKE ? ORDER BY id DESC LIMIT 50');
          $st->execute(['%'.$q.'%','%'.$q.'%','%'.$q.'%']);
          $rows=$st->fetchAll();
        } else {
          $rows=$db->query('SELECT * FROM tracks ORDER BY id DESC LIMIT 25')->fetchAll();
        }
        echo json_encode(['success'=>true,'data'=>$rows], JSON_UNESCAPED_UNICODE); 
        exit;
      }
...
<title>Админка — Ultra Fast Windows</title>
<style>
...
<body>
<header>
  <h1>Админка (Ultra Fast Windows)</h1>
  <p>Максимально быстрая версия для Windows</p>
</header>
<div class="bar">
...
<script>
const api='index_ultra_fast.php?api=1';
