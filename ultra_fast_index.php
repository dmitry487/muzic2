...
<body>
    ...
    <script>
        // Ultra fast version using simplified API
        ...
        // Simple auth check using fast API
        async function initAuth() {
            try {
                const res = await fetch('/muzic2/fast_api.php?action=user', { credentials: 'include' });
        ...
        async function login(username, password) {
            try {
                const res = await fetch('/muzic2/fast_api.php?action=login', {
                    method: 'POST',
        ...
        // Render home page using fast API
        async function renderHome() {
            const mainContent = document.getElementById('main-content');
            mainContent.innerHTML = '<div class="loading">Загрузка главной...</div>';
            
            try {
                // Load tracks and albums in parallel using fast API
                const [tracksRes, albumsRes] = await Promise.all([
                    fetch('/muzic2/fast_api.php?action=tracks', { credentials: 'include' }),
                    fetch('/muzic2/fast_api.php?action=albums', { credentials: 'include' })
                ]);
        ...
        // Render my music page using fast API
        async function renderMyMusic() {
            ...
            try {
                const res = await fetch('/muzic2/fast_api.php?action=playlists', { credentials: 'include' });
