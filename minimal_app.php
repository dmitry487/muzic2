...
<body>
    ...
    <script>
        ...
        async function checkUser() {
            try {
                const res = await fetch('/muzic2/minimal_api.php?action=user');
        ...
        async function logout() {
            await fetch('/muzic2/src/api/logout.php');
            user = null;
            updateUserPanel();
            loadHome();
        }
        ...
        async function openPlaylist(playlistId, playlistName) {
            ...
            try {
                const res = await fetch(`/muzic2/minimal_api.php?action=playlist_tracks&playlist_id=${playlistId}`);
                const data = await res.json();
